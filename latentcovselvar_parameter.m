function [Z, history] = latentcovselvar_parameter(D, mu_init, alpha, beta, t_inc, t_dec, var_par)
...
for k = 1:MAX_ITER
    ...
    if history.r_norm(k) > var_par*history.s_norm(k)
        mu = mu/t_inc;
    end
     
    if history.s_norm > var_par*history.r_norm(k)
        mu = mu*t_dec;
    end
